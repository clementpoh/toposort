...
/* Find the node equal to key in list, returns NULL if not found */
List find(bool (*eq)(void *key, void *node), void *key, List list) {
    return !list
        ? NULL
        : eq(key, list->data)
            ? list
            : find(eq, key, list->next);
}

/* Removes and returns the element equal to key in list,
 * returns NULL if not found */
void *del(bool (*eq)(void *key, void *node), void *key, List *list) {
    return !*list
        ? NULL
        : eq(key, (*list)->data)
            ? pop(list)
            : del(eq, key, &(*list)->next);
}
...
/* Print list in reverse by applying print to each node that is not NULL */
void print_list_rev(void (*print)(FILE *f, void *data), FILE *f, List list) {
    if (list) {
        print_list_rev(print, f, list->next);
        print(f, list->data);
    }
}
